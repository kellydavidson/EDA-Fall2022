...
Rainfall_BR_processed <-
  Rainfall_BR %>% 
  rename( Rainfall = Rainfall....MM.) %>% #renaming the rainfall column
  separate(Statistics,c("Null","Month","Null2")," ") %>% #similar to text to column in excel
  mutate( Date = my(paste0(Month,"-",Year))) %>% #creating a column with month only
...
print(BRA_plot_rainfall)

#no trend for rain over time, but yes seasonality
...
#positive trend for temp over time and seasonality
...
#seasonality explains more than the trend

BRA_temp_decomp <- stl(BRA_temp_ts, s.window = "periodic")
plot(BRA_temp_decomp)
#can see an increasing trend; both trend and seasonality have equal weight
