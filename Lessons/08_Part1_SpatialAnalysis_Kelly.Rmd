...
#Explore
class(GasPlants) #sf object means we can do spatial analysis - it has geometry column
names(GasPlants)
str(GasPlants$geometry)

#View using sf plot - active or inactive
plot(GasPlants['status'],pch=16) 

#View using ggplot
ggplot(data=GasPlants) + geom_sf()

#View using mapview - also pulls in the attributes
mapview(GasPlants)
...
#Convert to a spatial dataframe - creates new geometry column, takes out long & lat columns
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), #assign x (longitude) and y (latitude)
           crs=4326)
...
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) #input the coordinate location in lat & long, but it does not know where to put it
class(sfg_GraingerHall)
mapview(sfg_GraingerHall)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #crs (coordinate reference system) tells it lat & long (creates a simple features column)
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe to name the location - adds an attribute
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) #expand buffer by 1 degree (1 degree of latitude and 1 degree of longitude)

#View the result - we need to define a projection
mapview(the_buffer)
```
...
EPSG is a library or catalog of coordinate reference system
...
#Transform the datasets to use UTM 17 N, WGS84 in order to measure distance in meters
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #changes the projection (or transform) the dataset into eastings and northings (UTM)
GasPlants.utm <- st_transform(GasPlants,32617)

#Have a look at the coordinate associated with the geometries to confirm we are no longer in angular degrees, we are in meters
EPAair.sf.utm$geometry[1]

#Plot the data - axis still written in degrees, but lines now have a bend to them because they are projected
ggplot() +
  geom_sf(data = EPAair.sf.utm,color='blue') + 
  geom_sf(data = GasPlants.utm,color='red',shape='O') 

#Create a distance matrix between the point in each dataset - computes distance between 2 datasets
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) #pairwise makes it a matrix

#Compute the mean of each row (each EPA site)
mean_distances <-  rowMeans(distance_matrix)

#Add as a column to the epa sites - binding to the EPA dataset
EPAair.sf.utm  <-  EPAair.sf.utm %>% 
  bind_cols(mean_distances)

ggplot() +
  geom_sf(data = EPAair.sf.utm) 
```
