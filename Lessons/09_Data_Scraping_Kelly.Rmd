...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>%  #this is what we copied & pasted from the webpage
  html_text()
the_registrant
```
...
df_withdrawals <- data.frame("Month" = rep(1:12), #this creates a range from 1 to 12
                             "Year" = rep(2000,12), 
...
the_year <- 2015
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 removes spaces
...
ggplot(df_withdrawals,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste(the_year,the_registrant),
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
...
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping! This creates a pause when scraping
...
the_years = rep(2018:2020) #this creates a vector of a sequence from 2018-2020
my_facility = '0004-0001'

#Use lapply to apply the scrape function
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it, #repeats the function "scrape.it" for each value in "the_years"
                  the_facility=my_facility) #means this variable will be static

#OR, use purrr's map function - this is the same as lapply, just a different way to do it
the_dfs <- map(the_years,scrape.it,the_facility=my_facility)

#Conflate the returned dataframes into a single dataframe
the_df <- bind_rows(the_dfs) #bind_rows is the tidyverse version of "r_bind"
...
the_facility_ids_subset <-  sample(the_facility_ids,20) #select 20 sites IDs randomly - so we don't overload the server
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #use Map2 to iterate through 2 values in parallel - THEY MUST BE THE SAME SIZE
