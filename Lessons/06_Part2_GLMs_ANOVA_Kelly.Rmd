...
summary(Litter.Totals$plotID) #number of occurances at each site - not equivalent, but we can still run the 1 sided ANOVA test
...
# shapiro.test() performs the Shapiro-Wilk test of the null hypothesis that data follows a normal distribution
...
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass) #data does not follow a normal distribution
...
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID) #p value is less than 0.05 so reject the null, variance is not equal

#results: reject null i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) #anova is similar to lm function
