...
# Notice that other lake names didn't go away, even though they have zero values, use the droplevels function to fix that
...
#use as.Date function again to fix formatting, use lower case y because only 2 digits for the year
...
NTL.nutrient.data.PeterPaul <- 
  ...
  mutate(month = month(sampledate)) %>% #mutating to create a month column
  select(lakeid:daynum, month, sampledate:comments) %>% #selecting all the columns, reordering the columns
  drop_na(depth) #excluding all the "na's" in the depth column, in order to check this, you can input "summary(NTL.nutrient.data.PeterPaul$depth)"
...
# Remove columns that are not of interest for analysis by stating the range (from --> to) you want to keep - this explains two ways to do it!
#option 1 (on phys data)
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck)
#option 2 (on nutrient data)
...
'gather' is being replaced by 'pivot_longer' --> transforming columns into rows, making your data longer
'spread' is being replaced by 'pivot_wider' --> transforming rows into columns, making your data wider
...
#transforming 4 columns into 2 columns - creating 2 columns "nutrient" and "concentration" - creates more rows as a result --> 'pivot_longer'
...
#removing rows with "na" in the concentration column - this isn't giving us any helpful information
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))

#how many observations for each count in the nutrient column
count(NTL.nutrient.data.PeterPaul.gathered, "nutrient")

#store the dataset now that we have created a useful, tidy dataset
...
# Spread nutrient data into separate columns using pivot_wider - think of it as the opposite of 'pivot_longer'
...
# Opposite of 'separate' is 'unite' if you get a dataset with separate columns for year, month, day
...
```{r}
#joining phys and nutrient data sets (all rows and all columns, and will fill in values without numbers with "na")
...
```{r}
#tedious process, so instead, used the 'list.files' function
...
```{r}
#picking out similar columns and joining two datasets based on these same columns
...
#removing (by using 'select' function) columns that aren't of use to us 
#select function only applies to columns
...
NTL.PeterPaul.summaries <- 
  ...
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #removing na's for 3 columns - temperature_C, tn_ug, and tp_ug
  summarise(meantemp = mean(temperature_C), #state summaries you want
