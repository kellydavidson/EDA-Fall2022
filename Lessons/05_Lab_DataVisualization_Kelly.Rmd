...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #adding text expression using 'expression(paste)'
print(Plot_text)
...
# 1. 
# Plot surface temperatures by day of  year. - filter by removing depth == 0
# Color your points by year, and facet by lake in two rows. - add category
# Change the ylab name - 1 plot for each lab, change the y-axis name

PeterPaul.chem.nutrients <--
  PeterPaul.chem.nutrients %>% 
  filter(depth == 0)

ggplot(PeterPaul.chem.nutrients, filter(depth==0)) +
...
PPex1 <-
  ...
  geom_point(aes(color = year4)) +
  facet_wrap(var(lakename),nrow=2)


#2. 
# Plot temperature by date. Color your points by depth. - using full dataset
# Change the size of your point to 0.5
# Change the color palette to magma (divergent palette) and play with direction (+- 1), which one makes more sense?
...
EPAex3 <-
  ...
  geom_point(aes(color=meanLatitude), alpha=0.5) +
...
ggplot(EPAair, aes(x=as.factor(Year), y=PM2.5)) +
  geom_violin()
#violin plot is the best for this
