...
epa_pm25_sites_sf <- read_csv('./Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #converting into a spatial data frame, specifying x & y values, and specifying projection with crs

#Inspect the object - it is a sf object so we can do spatial analysis on it
class(epa_pm25_sites_sf)

#What is its CRS again? coordinate reference system
...
# reading in shapefiles from Data -> Spatial folder using "st_read"
counties_sf<- st_read('./Data/Spatial/cb_2018_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties - NC FIPS code is 37
...
#Reveal the extent of this dataset via the st_bbox() function - creates a bounding box - lower left and upper right coordinates
st_bbox(counties_sf)
...
#Create a map
mapView(huc8_sf, col.regions = "orange") + mapView(huc8_upperneuse_sf, col.regions = "purple")
...
```{r load.geojson.data}
#JSON constructed for sharing data online - way to pull data from online into R
...
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
  count(TYPE, sort=TRUE) #this will count the number of types

#Most are solar farms; let's remove those
nc_powerplants_sf <- nc_powerplants_sf %>% 
  filter(TYPE != "SOLAR PHOTOVOLTAIC") #filter for all that are NOT solar photovoltaic
...
#Read the 2018 SVI county-level dataset for NC - notice still no geometry column
svi2018_nc_raw <- read.csv(
  'https://svi.cdc.gov/Documents/Data/2018_SVI_Data/CSV/States_Counties/NorthCarolina_COUNTY.csv',
  colClasses = c('FIPS' = 'factor')) %>% #specify how to input column names (ie as a factor) ensures you don't loose numbers at the beginning too
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#Join the SVI attributes to the county spatial features - "database join" - it looks for a common attribute to ensure things are lined up
counties_sf_join <-  merge(x = counties_sf, #specify x
                           y = svi2018_nc_raw, #specify y
                           by.x = "GEOID", 
                           by.y = "FIPS" )
...
mapview(counties_sf_join, 
  ...
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #mapping particulate matter
...
wind_energy_sf_join <- merge(x = huc8_sf,
                             y = wind_energy_huc8,
                             by.x = "HUC_8",
                             by.y = "HUC_8")

#View the outputs
mapView(wind_energy_sf_join, zcol = "kWhkm2day")
