...
library(zoo)
#install.packages("Kendall") needed to run a specific stationary test
library(Kendall)
...
* **Data gaps**: Data are not collected at regular intervals, necessitating *interpolation* between measurements. - DO NOT REMOVE NAs
...
```{r}
#selecting the column we want to analyze, working with daily data
...
```{r}
?stl #calls up help window
# Generate the decomposition, do not run decom with more than 2 variables
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

# Visualize the decomposed series - line plot can help ID gaps and overall trends and potential seasonal component
#bars to the right indicate the varaibility of the data, smaller bar = component does not explain much of the data, larger bar = component explains a lot of the data
...
EnoDischarge.late_Components <- #adding 2 columns - observed and date so we can plot them altogether
...
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) + #adding a line, observed 
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") + #adding a line, trend
  geom_hline(yintercept = 0, lty = 2) + #adding a horizontal line at 0
...
## Trend analysis - we will focus on the monotonic trends
...
### Monotonic trend analysis, continued - will either be upward or downward
...
# Run SMK test - data must be a time series object (not data frame)!!
