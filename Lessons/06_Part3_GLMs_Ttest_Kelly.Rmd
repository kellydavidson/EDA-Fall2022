...
### One-sample t-test
The object of a one sample test is to test the null hypothesis that the mean of the group is equal to a specific value. The alternative hypothesis is that the mena of the group is NOT equal to that specific value
...
> Answer: 
H0 (null): mu >= 50 **equals symbol must always be on the null hypothesis
Ha (alternative): mu < 50

```{r}
summary(EPAair$Ozone) #mean = 40.88 and median = 40
length(EPAair$Ozone) #8976 observations - the mean of our sample is a good representation of the population

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") #t test function
O3.onesample #p value is less than 0.05 so reject null - pop mean IS less than 50

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, #creating a vertical line at 50
             color = "#238b45", lty = 2, size = 0.9) + #most of the observations fall to the LEFT of the vertical line
  scale_x_continuous(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0))
print(Ozone.plot)

# Format this same scenario as a GLM
O3.onesample2 <- lm(Ozone ~ 1, EPAair) #no explanatory variable, just an intercept term
O3.onesample2
```
...
For example, we might ask ourselves (from the EPA air quality processed dataset): 
Are Ozone levels different between 2018 and 2019? <-- research question
...
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) #year is the categorical, explanatory variable
O3.twosample #p value is less than 0.05, reject null hypothesis - the two means ARE NOT the same

# Format as a GLM - need to change the question a bit - "is year a predictor of ozone levels?"
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year)
summary(O3.twosample2) #p value is less than 0.05 - reject null - year is a predictor of ozone levels
