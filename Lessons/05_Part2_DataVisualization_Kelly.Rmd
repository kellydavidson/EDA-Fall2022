...
print(O3plot) #default background is gray and points are black
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #gray lines and axes
print(O3plot3)
...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), #creating black axis titles and making the text size larger
        legend.position = "top") #default legend placement is to the right
...
theme_set(mytheme) #set the theme to be a default theme for an entire chunk
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) #creating a horizontal line +
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) #adding points to the scatterplot + changing transparency
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left align, this adds "good" to the plot in bold
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #changing the scale along the x-axis, just plots a section of the data, creating breaks in the plot and using month abbreviations, not numbers
  ylab(expression("O"[3]* " AQI Value")) #changes the axis titles +
...
scales::show_col(colormap(colormap = colormaps$viridis, nshades = 16)) #shows you the different colors in this package
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + #changing the transparency and size of points
  ...
  scale_color_viridis(option = "magma", direction = -1) + #choosing "magma" as the theme
...
In situations where facets don't fill our needs to place multiple plots on a page, we can use the package `cowplot` to arrange plots. The `plot_grid` function is extremely flexible in its ability to arrange plots in specific configurations. A useful guide can be found here: https://cran.r-project.org/web/packages/aplot/vignettes/introduction.html.
...
#putting two graphs on the same window, set number of rows to "2" 
#"rel_heights = " allows you to change the relative height of each plot
