...
#2. Building a ggplot theme and setting it as the default theme
A06_theme <-
...
> Answer:
H0: During the month of July, there is no correlation/relationship between mean lake temperature and depth; meaning the intercept and slope coefficients are equal to zero
Ha: During the month of July, there is a relationship between mean lake temperature and depth; meaning the intercept and slope are not equal to zero
...
tempbydepthplot <-
  ...
  xlab(expression("Depth")) #renaming the x-axis
...
> Answer: The scatterplot of temperature by depth imply that there is a negative correlation between temperature (in degrees C) and depth during the month of July.
...
> Answer: p-value is very small, less than 0.05 - so reject null hypothesis - there is a relationship between temp and depth
r-squared value is 0.7387 - ~74% of the variability in temperature is explained by depth
...
#10. Running a multiple regression on year4, daynum, & depth
...
> Answer: AIC method suggests we use year4, daynum, and depth as explanatory variables to predict temperature
new r-squared value is ~74% - so more of the variability in temp is explained by year4, daynum, and depth
...
>Answer: 
same mean as Peter Lake: Central Long Lake, Crampton Lake, East Long Lake, Hummingbird Lake, Tuesday Lake, West Long Lake
