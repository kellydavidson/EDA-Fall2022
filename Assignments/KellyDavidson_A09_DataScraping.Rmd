...
Max_daily_withdrawal_plot <- ggplot(Durham_LWSP_df2, aes(x = Date, y = Max.withdrawals.mgd)) +
  geom_line() +
  ylab("Maximum Daily Withdrawal (MGD)")
...
scrape.it <- function(pswid, year){
  ...
  Durham_LWSP_url <- read_html(paste0("https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=2021", pswid, '/', year))
  ...
  water_system_name <- Durham_LWSP_url %>% html_node(water_system_tag) %>% html_text()
  pswid_name <- Durham_LWSP_url %>%  html_node(pswid_tag) %>%  html_text()
  ownership_name <- Durham_LWSP_url %>%  html_node(ownership_tag) %>%  html_text()
  max_withdrawals <- Durham_LWSP_url %>%  html_node(max_withdrawals_tag) %>%  html_text()
