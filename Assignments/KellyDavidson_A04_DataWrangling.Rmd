...
```{r, message = FALSE}
#1. Checking working directory, loading 'tidyverse' and 'lubridate' packages, and uploading 4 EPA air datasets
getwd()
library(tidyverse)
library(lubridate)
library(dplyr)
...
```{r}
#3. Change the "Date" columns to dates instead of factors in each dataset
...
#4. Selecting the columns (Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE) for each dataset
EPAair_PM25_2019 <-
  EPAair_PM25_2019 %>% 
...
#5. For the two PM2.5 datasets (2018 & 2019), fill in all cells within the "AQS_PARAMETER_DESC" column with "PM2.5"
EPAair_PM25_2019 <-
  EPAair_PM25_2019 %>% 
...
#11. 
write.csv(EPAair_18_19, row.names = FALSE, file = "./Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv")
